...
| # | Domínio | Responsabilidade | Entidades candidatas | Hoje |
|---|---|---|---|---|
| 5 | **Paciente / Cidadão** | Identidade da pessoa que usa a rede pública de saúde. | `Paciente` (identificação, contatos, endereço, CNS) | ⏳ Próxima onda — modelo revisado em [`adr/DER.md`](./adr/DER.md) |
| 4 | **Atendimento** | Momento em que o cidadão entra na rede para receber um serviço. | `Atendimento`, `TipoAtendimento`, `Acolhimento` | ⏳ Próxima onda — modelo revisado em `adr/DER.md` |
...
| 🔵 **Operação Assistencial** | #5 Paciente → #4 Atendimento → #7 Agendamento → #6 Prontuário (agregação), depois #8 Enfermagem, #9 Farmácia, #10 Laboratório, #11 Regulação, #12 Leitos, #16 Transporte, conforme requisito real | ⏳ Próxima — modelo de dados da primeira fatia (Paciente/Atendimento/Agendamento/Prontuário) já revisado em `adr/DER.md` |
