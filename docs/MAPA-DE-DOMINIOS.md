...
| # | Domínio | Responsabilidade | Entidades candidatas | Hoje |
|---|---|---|---|---|
| 20 (parte) | **Identidade e Segurança** | Autenticação, papéis, permissões e escopo por contexto organizacional. | `Usuario`, `Papel`, `Permissao`, `EscopoAcesso`, `AtribuicaoAcesso` | ⏳ Implementação adiada por decisão explícita (ADR-0006, reafirmada na ADR-0039) — revisitar antes de produção com dado real de paciente. **Desenho já decidido** pela [ADR-0054](./adr/0054-modelo-de-identidade-autorizacao-e-auditoria.md) (RBAC + escopo organizacional + catálogo de permissões como dado, substituindo as 4 roles fixas da ADR-0006) |
