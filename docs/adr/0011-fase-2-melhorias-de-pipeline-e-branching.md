...
`qaa`/`homologacao`/`main`, auto-merge total, `enforce_admins: true` sem nenhum bypass). Esta ADR separa o
...
4. **Validação de nome de branch na CI** — um step que rejeita nomes fora do padrão (`feature/`,
   `fix/`, `hotfix/`, `developer`, `qaa`, `homologacao`, `main`). Só faz sentido depois do item 2 existir
   de fato.
...
2. ~~Nome de branch `qaa`~~ **Resolvido — branches renomeadas.** Ao criar as tarefas no Jira
   (AQUAQE-225), constatado que o próprio fluxo de status do projeto AQUAQE já usa "QAA" e
   "HOMOLOGACAO" como nomes reais de etapa — não era erro de digitação no documento de
   referência, era convenção já em uso no processo do time. Confirmado com o usuário: as branches
   do GitHub `qa` e `cert` foram renomeadas para `qaa` e `homologacao` (via o endpoint oficial de
   rename do GitHub — PRs abertos e proteção de branch clássica migraram automático; o Ruleset da
   seção "Atualização" abaixo precisou de ajuste manual). Ver AQUAQE-242 e ADR-0010.
...
- **Alvo:** as 4 branches (`developer`, `qaa`, `homologacao`, `main`) — clarificado com o usuário que a
