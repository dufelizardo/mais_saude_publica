...
/**
 * Testes do ConsultaController (Assistência — ver ADR-0039/ADR-0043). Mesmo padrão de engenharia
 * de teste dos demais controllers: SEM {@code @Transactional} na classe, limpeza manual via
 * {@code @AfterEach} incondicional para Consulta/Atendimento (único teste que os cria — mesmo
 * raciocínio do AtendimentoControllerTest, evita navegar associações LAZY fora de transação).
 */
@SpringBootTest
...
class ConsultaControllerTest {
    ...
    private static final String PREFIXO_NOME_TESTE = "Consulta Teste - ";
    private static final String PREFIXO_CPF_TESTE = "44433322";
    ...
    void limparDadosDeTeste() {
        // Consulta/Atendimento não têm campo de teste próprio para filtrar sem navegar associações
    ...
    private String corpoConsulta(UUID atendimentoId, String profissionalMatricula, String tipoConsulta) {
        return """
                ...
                """.formatted(atendimentoId, profissionalMatricula, tipoConsulta);
    }
    ...
    void deveRetornarNotFoundQuandoAtendimentoNaoExiste() throws Exception {
        ...
        mockMvc.perform(post(CONSULTA_URL)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(corpoConsulta(UUID.randomUUID(), matricula, "PRIMEIRA")))
    ...
    void deveBuscarPorId() throws Exception {
        ...
        mockMvc.perform(get(CONSULTA_URL + uuid))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.tipoConsulta").value("RETORNO"))
                .andExpect(jsonPath("$.diagnostico").value("Cefaleia tensional"));
    }

    @Test
    void deveRetornarNotFoundAoBuscarIdInexistente() throws Exception {
        mockMvc.perform(get(CONSULTA_URL + UUID.randomUUID()))
                .andExpect(status().isNotFound());
    }
