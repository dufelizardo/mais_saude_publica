...
class CategoriaSalarialControllerTest {
    ...
    void deveAtualizarComSucesso() throws Exception {
        UUID uuid = criarCategoria(PREFIXO_NOME_TESTE + "Original");
        String nomeAtualizado = PREFIXO_NOME_TESTE + "Atualizada";
        String body = """
                {
                  "nome": "%s",
                  "convencaoColetiva": "SINDSAUDE-SP 2027"
                }
        ...
        mockMvc.perform(patch(BASE_URL + uuid)
                        ...
                        .content(body))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.message").value("Categoria salarial atualizada com sucesso!"));

        mockMvc.perform(get(BASE_URL + uuid))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.nome").value(nomeAtualizado))
                .andExpect(jsonPath("$.convencaoColetiva").value("SINDSAUDE-SP 2027"));
    }
    ...
    void deveRetornarNotFoundAoAtualizarIdInexistente() throws Exception {
        String body = """
                ...
                """.formatted(PREFIXO_NOME_TESTE + "Inexistente");
