...
public class ProcedimentoController {
    ...
    @Operation(summary = "Atualiza um procedimento",
            description = "Substitui os campos editáveis por inteiro — cobre tanto corrigir dados quanto avançar o status (AGENDADO/REALIZADO/CANCELADO).",
            tags = "Procedimento")
    @ApiResponse(responseCode = "200", description = "Success:", content = {
            @Content(mediaType = "application/json", array = @ArraySchema(
                    schema = @Schema(implementation = SuccessResponseDto.class)
    ...
    @ApiErrorResponsesMutacao
    public ResponseEntity<SuccessResponseDto> update(@PathVariable UUID uuid, @Valid @RequestBody ProcedimentoRequestDto dto) {
        ProcedimentoResponseDto responseDto = service.atualizar(uuid, dto);

        String successMessage = "Procedimento atualizado com sucesso!";
